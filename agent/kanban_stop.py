...
def build_kanban_stop_nudge(
    *,
    ...
    return (
        ...
        # narrated assistant text: the summary is what the USER reads, nothing
        # else reaches them. A worker had just written the full answer as plain
        # text (10 invoices with dates and amounts), got nudged, and handed
        # kanban_complete "J'ai listé les 10 dernières factures" — the content
        # died in its own log (2026-08-11). Upstream never says what `summary`
        # should hold because upstream's summary is a handover note between
        # agents; ours is the answer itself.
        "3. The `summary` is the ANSWER ITSELF, not a report about it — it is "
        "the only thing the user will ever see. If your previous plain-text "
        "reply already contains the deliverable, carry its FULL useful content "
        "over verbatim (facts, rows, figures, conditions, dates and links). "
        "Writing \"I have listed X\", \"I explained\" or \"I found\" delivers "
        "nothing: the content stays in your log and the user reads a sentence "
        "about work they cannot see. Cap a long list around 30 rows and state "
