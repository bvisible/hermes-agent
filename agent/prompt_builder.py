...
KANBAN_GUIDANCE = (
    ...
    #//// Neoffice — appended to upstream rule 5 (was on the pre-v0.21 rule we
    #//// replaced). Our fork also enforces this in CODE (conversation_loop
    #//// terminal-break re-reads the task status after a terminal kanban tool),
    #//// but the prompt keeps the model from even trying: workers that kept
    #//// acting after a successful kanban_complete/kanban_block created
    #//// duplicate purchase orders (2026-08-21).
    ...
    "## Do NOT\n"
    "\n"
    "- You have NO terminal, NO `execute_code`, NO filesystem, and NO "
    "Python/SQL runtime — those do not exist in your schema. NEVER write, "
    "save, or 'execute a script' to obtain or compute data; your ONLY ways "
    "to act are your `frappe_*` domain tools and the `kanban_*` tools. If "
    "your `frappe_*` tools cannot answer (no tool fits, or the data is out "
    "of scope), call `kanban_block(reason=...)` IMMEDIATELY and stop — do "
    "NOT re-call `kanban_show` or re-plan in a loop until the iteration "
    "budget runs out (that wastes minutes and times the user out).\n"
