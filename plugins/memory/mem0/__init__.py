    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        def _sync():
            ...
            try:
                ...
                # default: that value is tuned for a 512-token embedder and would have cut 30
                # of the 364 memories stored on osiris (8 %, the longest and most useful ones:
                # worker answers). Ours is qwen3-embedding:8b, measured 2026-09-13: 20 000
                # chars (4 910 tokens) embed in 9.4 s, 60 000 get a 503 from the shared
                # embedding backend. Stored turns peak at 961 chars, so 8 000 never touches a
                # real turn; it only keeps one paste from being dropped on a 503 or recalled
                # whole into the system prompt. An explicit `sync_max_chars` in mem0.json
                # still wins, as upstream intends.
                _raw_cap = int((self._config or {}).get("sync_max_chars") or 8000)
                kept = [{**m, "content": _truncate_for_sync(m["content"], _raw_cap)} for m in kept]
                backend.add(
                    kept,
