    ...
    def _flush_messages_to_session_db_unlocked(
        ...
        try:
            ...
            for _msg_idx in range(_scan_start, len(messages)):
                ...
                # osiris-poc). Runs AFTER the api_content sidecar is computed above, so the
                # sidecar keeps wire-truth (replay fidelity) while the SAVED transcript matches
                # what the user receives. The apply_branding_filter_state runtime patch detects
                # the marker line below and skips, so this commit + that patch never double-apply.
